...
public class Main { //Zoo>>>36) создаем клиентский класс
    public static void main(String[] args) { // 37) создаем точку входа
        ...
        zoopark.addAnimals(List.of(new Cat("Barsik"), new Dog("Sharik"), new Rabbit("Whik"), new Cow("Murka"), new Duck("Krya"), new Butterfly("Yellow"), new Seal("Simmens"), new Walrus("Hsora"))); // через функцию добавления животных добавляем их в класс зоопарк // Duck >>>70) Добавляем утку >>> Zoo 71) // Butterfly >>> 84) добавляем бабочку
        ...
        System.out.println("----------------------RunAll-----------------------"); // Zoo 73) создаем разделитель консоли
        for (Runable item : zoopark.getRunable()){ // 74) создаем метод вывода в консоль 
            System.out.println(item);
        }
        System.out.println("-------------------FlyAll--------------------------"); // 75) создаем разделитель консоли
        for (Flyable item : zoopark.getFlyable()){ // 76) создаем метод вывода в консоль >>> 77) Zoo
            System.out.println(item);
        }
        System.out.println("---------------------WinnerRan------------------------"); // Zoo>>>78) создаем разделитель
        System.out.println(zoopark.getWinner()); // 79) Создаем метод вывода чемпиона по бегу в консоль >>> 80) Butterfly

        System.out.println("----------------------WinnerFly-----------------------"); // 85) создаем разделитель
        System.out.println(zoopark.getWinnerFly()); // 86) Создаем метод выводе чемпиона по скорости полета в консоль >>> 87 Zoo

        System.out.println("---------------------WinnerFloating------------------------"); // Zoo>>>78) создаем разделитель
        System.out.println(zoopark.getWinnerFloating()); // 79) Создаем метод вывода чемпиона по бегу в консоль >>> 80) Butterfly
