...
git commit -a если для lubuntu то ввести текст комментария, затем Ctr + s, далее вроде нажать esc, затем enter, затем на предложение сохранить нажать Y точно не помню, но работало, сохраняет все изменения, в другой раз сработало для сохнения и выхода Ctr + x.
