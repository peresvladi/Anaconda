...
1) git reflog (после этой команды, по результатам изучения выведенной справки определяемся к какому коммиту надо откатываться)
2) git revert HEAD~8 (8 - это номе коммита к которому как я понял происходи откат (в справке после команды reflog указан немного иначе: HEAD@{8}:), HEAD~8 такой вид как пишут в нете это относительная ссылка по отношению к коммитам, а) HEAD@{8} относительно действия.
3) git push --force (как пишут в нете форс говорит серверу, что в нем мусор, поэтому не надо мержить, просто забери, что дают и сделай все как у меня)
